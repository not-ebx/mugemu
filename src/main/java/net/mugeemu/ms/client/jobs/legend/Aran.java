...
public class Aran extends Job {
    ...
    private void giveAdrenalinRushBuff(TemporaryStatManager tsm) {
        ...
        if (chr.hasSkill(ADRENALINE_RUSH)) {
            ...
            tsm.sendSetStatPacket();
            EventManager.addEvent(this::setComboCountAfterAdrenaline, si.getValue(SkillStat.time, slv), TimeUnit.SECONDS);
        }
    }

    private void comboAfterAdrenalin() {
        TemporaryStatManager tsm = chr.getTemporaryStatManager();
        tsm.getOption(CharacterTemporaryStat.ComboAbilityBuff).nOption = 500;
    }
    ...
    public void handleAttack(Client c, AttackInfo attackInfo) {
        ...
        if (hasHitMobs) {
            if(chr.hasSkill(ADRENALINE_RUSH) && tsm.getOption(CharacterTemporaryStat.ComboAbilityBuff).nOption > 999 && !tsm.hasStat(
        ...
        switch (attackInfo.skillId) {
            case FINISHER_HUNTER_PREY:
                int t = si.getValue(SkillStat.subTime, slv);
                o1.nOption = 1;
                o1.rOption = skillID;
                o1.tOption = t;
    ...
    private void incrementComboAbility(TemporaryStatManager tsm, AttackInfo attackInfo) {
        ...
        if(tsm.hasStat(CharacterTemporaryStat.ComboAbilityBuff)) {
            ...
            if (amount < comboInfo.getValue(SkillStat.s2, chr.getSkill(COMBO_ABILITY).getCurrentLevel())) {
                for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                    amount++;
                }
            }
    ...
    public void snowCharge(AttackInfo attackInfo) {
        ...
        if(tsm.getOptByCTSAndSkill(CharacterTemporaryStat.WeaponCharge, SNOW_CHARGE) != null) {
            for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                ...
                MobTemporaryStat mts = mob.getTemporaryStat();
                o1.nOption = (mob.isBoss() ? -(si.getValue(SkillStat.q, slv) / 2) : - si.getValue(SkillStat.q, slv));
