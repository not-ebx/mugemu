...
public class Shade extends Job {
    ...
    public void handleBuff(Client c, InPacket inPacket, int skillID, byte slv) {
        ...
        switch (skillID) {
            case FOX_SPIRITS:
                ...
                o1.rOption = skillID;
                o1.tOption = 0;
                tsm.putCharacterStatValue(CharacterTemporaryStat.HiddenPossession, o1);
                break;
            ...
            case SPIRIT_WARD:
                ...
                o1.rOption = skillID;
                o1.tOption = si.getValue(SkillStat.time, slv);
                tsm.putCharacterStatValue(CharacterTemporaryStat.SpiritGuard, o1);
                spiritWardTimer = System.currentTimeMillis() + (si.getValue(SkillStat.time, slv) * 1000);
                break;
            ...
            case SPIRIT_BOND_MAX:
                ...
                tsm.putCharacterStatValue(CharacterTemporaryStat.IndieBooster, o4);
                o5.nReason = skillID;
                o5.nValue = si.getValue(SkillStat.indieIgnoreMobpdpR, slv);
                o5.tStart = (int) System.currentTimeMillis();
                o5.tTerm = si.getValue(SkillStat.time, slv);
                tsm.putCharacterStatValue(CharacterTemporaryStat.IndieIgnoreMobpdpR, o5);
                break;
        }
    ...
    public void handleAttack(Client c, AttackInfo attackInfo) {
        ...
        switch (attackInfo.skillId) {
            ...
            case BOMB_PUNCH_FINAL:
                ...
                if (Util.succeedProp(bpi.getValue(SkillStat.prop, bombPunchslv))) {
                    for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                        Mob mob = (Mob) chr.getField().getLifeByObjectID(mai.mobId);
                        if(mob == null) {
                            continue;
                        ...
                        if(!mob.isBoss()) {
                            ...
                            o1.tOption = bpi.getValue(SkillStat.time, bombPunchslv);
            ...
            case DEATH_MARK:
                ...
                for(MobAttackInfo mai : attackInfo.mobAttackInfo) {
                    ...
                    o1.nOption = 1;
                    o1.rOption = skill.getSkillId();
                    o1.tOption = si.getValue(SkillStat.dotTime, slv);
                    mts.addStatOptionsAndBroadcast(MobStat.DebuffHealing, o1);
                    mts.createAndAddBurnedInfo(chr, skill);
                }
                break;
            case SOUL_SPLITTER:     // TODO
                int duration = si.getValue(SkillStat.time, slv); //Split Duration & Timer on when to removeLife
                for(MobAttackInfo mai : attackInfo.mobAttackInfo) {
                    Mob mob = (Mob) chr.getField().getLifeByObjectID(mai.mobId);
                    if (mob == null) {
                        continue;
    ...
    private void createFoxSpiritForceAtom(int skillID) {
        TemporaryStatManager tsm = chr.getTemporaryStatManager();
        if (tsm.hasStat(CharacterTemporaryStat.HiddenPossession)) {
    ...
    public void handleHit(Client c, InPacket inPacket, HitInfo hitInfo) {
        TemporaryStatManager tsm = chr.getTemporaryStatManager();
        if(tsm.hasStat(CharacterTemporaryStat.SpiritGuard) && hitInfo.hpDamage > 0) {
            deductSpiritWard();
            hitInfo.hpDamage = 0;
            hitInfo.mpDamage = 0;
        }
    ...
    private void deductSpiritWard() {
        ...
        Option o = new Option();
        if (tsm.hasStat(CharacterTemporaryStat.SpiritGuard)) {
            int spiritWardCount = tsm.getOption(CharacterTemporaryStat.SpiritGuard).nOption;
            ...
            if(spiritWardCount <= 0) {
                ...
                tsm.putCharacterStatValue(CharacterTemporaryStat.SpiritGuard, o);
                tsm.sendSetStatPacket();
            }
        }
    }
