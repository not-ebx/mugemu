...
import static net.mugeemu.ms.enums.ForceAtomEnum.*;
...
public class Mechanic extends Citizen {
    ...
    public void handleBuff(Client c, InPacket inPacket, int skillID, byte slv) {
        ...
        switch (skillID) {
            case HUMANOID_MECH:
                ...
                o1.rOption = skillID;
                tsm.putCharacterStatValue(CharacterTemporaryStat.Mechanic, o1);
                o2.nOption = si.getValue(SkillStat.epad, slv);
                o2.rOption = skillID;
                o2.tOption = 0;
                tsm.putCharacterStatValue(CharacterTemporaryStat.PAD, o2);
                o3.nOption = si.getValue(SkillStat.emmp, slv);
                o3.rOption = skillID;
                o3.tOption = 0;
                tsm.putCharacterStatValue(CharacterTemporaryStat.EMMP, o3);
                o4.nOption = si.getValue(SkillStat.emhp, slv);
                o4.rOption = skillID;
                o4.tOption = 0;
                tsm.putCharacterStatValue(CharacterTemporaryStat.EMHP, o4);
                o5.nOption = si.getValue(SkillStat.indieSpeed, slv);
                o5.rOption = skillID;
                o5.tOption = 0;
                tsm.putCharacterStatValue(CharacterTemporaryStat.IndieSpeed, o5);
            ...
            case FULL_SPREAD:
                ...
                o1.rOption = skillID;
                o1.tOption = si.getValue(SkillStat.time, slv);
                tsm.putCharacterStatValue(CharacterTemporaryStat.BombTime, o1);
                break;
        }
        tsm.sendSetStatPacket();
    }
    ...
    public void healFromSupportUnit(Summon summon) {
        ...
        if (summonOwner.hasSkill(ENHANCED_SUPPORT_UNIT) || summonOwner.hasSkill(SUPPORT_UNIT_HEX)) {
            ...
            int healrate = si.getValue(SkillStat.hp, slv);
    ...
    private void spawnBotsNTotsSubSummons(Summon summon) {
        ...
        if((tsm.getOptByCTSAndSkill(CharacterTemporaryStat.IndieEmpty, BOTS_N_TOTS) != null) && chr.hasSkill(BOTS_N_TOTS)) {
    ...
    private int getHomingBeaconBulletCount() {
        ...
        for(int skillId : homingBeacon) {
            if(chr.hasSkill(skillId)) {
                ...
                SkillInfo si = SkillData.getSkillInfoById(skillId);
                byte slv = (byte) skill.getCurrentLevel();
                forceAtomCount += si.getValue(SkillStat.bulletCount, slv);
            }
        }
        if(tsm.getOptByCTSAndSkill(CharacterTemporaryStat.BombTime, FULL_SPREAD) != null) {
            forceAtomCount += chr.hasSkill(FULL_SPREAD) ? SkillData.getSkillInfoById(FULL_SPREAD).getValue(SkillStat.x, chr.getSkill(FULL_SPREAD).getCurrentLevel()) : 0;
        }
    ...
    private void applySupportUnitDebuffOnMob(int skillId) {
        TemporaryStatManager tsm = chr.getTemporaryStatManager();
        if(!chr.hasSkill(SUPPORT_UNIT_HEX) || tsm.getOptByCTSAndSkill(CharacterTemporaryStat.IndieEmpty, skillId) == null) {
            return;
        }
        Skill skill = chr.getSkill(skillId);
        ...
        for(Mob mob : field.getMobs()) {
            ...
            o.nOption = -suhInfo.getValue(SkillStat.w, chr.getSkill(SUPPORT_UNIT_HEX).getCurrentLevel()); // enhancement doesn't contain the debuff info
        ...
        supportUnitTimer = EventManager.addEvent(() -> applySupportUnitDebuffOnMob(skillId), si.getValue(SkillStat.x, slv), TimeUnit.SECONDS);
    }
            ...
            switch (skillID) {
                ...
                case OPEN_PORTAL_GX9:
                    ...
                    if (chr.hasSkill(ROBOT_MASTERY)) {
                        ...
                        duration *= 1 + (double) (robotMastery.getValue(SkillStat.x, chr.getSkillLevel(ROBOT_MASTERY)) / 100);
                    }
                ...
                case ADV_HOMING_BEACON: // 4thJob upgrade +5 -> 9
                    if(tsm.hasStat(CharacterTemporaryStat.Mechanic) && tsm.getOption(CharacterTemporaryStat.Mechanic).nOption <= 0) {
                        createHumanoidMechRocketForceAtom();
                    } else if (tsm.hasStat(CharacterTemporaryStat.Mechanic) && tsm.getOption(CharacterTemporaryStat.Mechanic).nOption == 1) {
                ...
                case ROCK_N_SHOCK:
                    ...
                    if (chr.hasSkill(ROBOT_MASTERY)) {
                        SkillInfo robotMastery = SkillData.getSkillInfoById(ROBOT_MASTERY);
                        summon.setSummonTerm((int) (summon.getSummonTerm() * (double) (1 + (robotMastery.getValue(
                            SkillStat.x, chr.getSkillLevel(ROBOT_MASTERY)) / 100))));
                    }
