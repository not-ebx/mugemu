...
import net.mugeemu.ms.client.jobs.adventurer.BeastTamer;
import net.mugeemu.ms.client.jobs.adventurer.Beginner;
import net.mugeemu.ms.client.jobs.adventurer.Magician;
import net.mugeemu.ms.client.jobs.adventurer.Warrior;
import net.mugeemu.ms.client.jobs.cygnus.BlazeWizard;
import net.mugeemu.ms.client.jobs.cygnus.Mihile;
import net.mugeemu.ms.client.jobs.cygnus.NightWalker;
import net.mugeemu.ms.client.jobs.legend.Phantom;
...
public abstract class Job {
	...
	public void handleAttack(Client c, AttackInfo attackInfo) {
		...
		if(tsm.getOptByCTSAndSkill(CharacterTemporaryStat.IgnoreMobDamR, RuneStone.LIBERATE_THE_RECOVERY_RUNE) != null) {
			...
			int healrate = recoveryRuneInfo.getValue(SkillStat.dotHealHPPerSecondR, recoveryRuneSLV);
		...
		switch (skillID) {
			case RuneStone.LIBERATE_THE_DESTRUCTIVE_RUNE:
				...
				o1.nValue = si.getValue(SkillStat.indieDamR, slv); //50% DamR
		...
		if (inPacket != null && isBuff(skillID)) {
			...
			if(chr.hasSkill(skillID) && si.getVehicleId() > 0) {
				...
				if(tsm.hasStat(CharacterTemporaryStat.RideVehicle)) {
					tsm.removeStat(CharacterTemporaryStat.RideVehicle, false);
				}
				tsb.setNOption(si.getVehicleId());
				tsb.setROption(skillID);
				tsm.putCharacterStatValue(CharacterTemporaryStat.RideVehicle, tsb.getOption());
				tsm.sendSetStatPacket();
			} else {
				...
				if (noviceSkill == 1085 || noviceSkill == 1087 || noviceSkill == 1090 || noviceSkill == 1179) {
					...
					if (field.isFly()) {
						...
						option.tOption = 0;
						tsm.putCharacterStatValue(CharacterTemporaryStat.Flying, option);
						tsm.sendSetStatPacket();
					}
				}
	...
	public void handleJoblessBuff(Client c, InPacket inPacket, int skillID, byte slv) {
		...
		switch (skillID) {
			case BOSS_SLAYERS:
				o1.nReason = skillID;
				o1.nValue = si.getValue(SkillStat.indieBDR, slv);
				o1.tStart = curTime;
				o1.tTerm = si.getValue(SkillStat.time, slv);
				tsm.putCharacterStatValue(CharacterTemporaryStat.IndieBDR, o1);
				break;
			case UNDETERRED:
				o1.nReason = skillID;
				o1.nValue = si.getValue(SkillStat.indieIgnoreMobpdpR, slv);
				o1.tStart = curTime;
				o1.tTerm = si.getValue(SkillStat.time, slv);
			...
			case MAPLERUNNER_DASH:
				...
				o1.tTerm = o2.tTerm = si.getValue(SkillStat.time, slv);
				o1.nValue = si.getValue(SkillStat.indieForceJump, slv);
				tsm.putCharacterStatValue(CharacterTemporaryStat.IndieForceJump, o1);
				o2.nValue = si.getValue(SkillStat.indieForceSpeed, slv);
				tsm.putCharacterStatValue(CharacterTemporaryStat.IndieForceSpeed, o2);
				break;
			...
			case Beginner.RECOVERY:
				o1.rOption = skillID;
				o1.tOption = 30;
				tsm.putCharacterStatValue(CharacterTemporaryStat.Restoration, o1);
				recoveryInterval();
	...
	public void handleHit(Client c, HitInfo hitInfo) {
		...
		if(chr.getStat(Stat.hp) <= hitInfo.hpDamage) {
			...
			else if (tsm.getOptByCTSAndSkill(CharacterTemporaryStat.ReviveOnce, NightWalker.DARKNESS_ASCENDING) != null ) {
				NightWalker.reviveByDarknessAscending(chr);
			}

			// Blaze Wizard - Phoenix Run
			else if (tsm.getOptByCTSAndSkill(CharacterTemporaryStat.ReviveOnce, BlazeWizard.PHOENIX_RUN) != null) {
				BlazeWizard.reviveByPhoenixRun(chr);
			}

			// Shade - Summon Other Spirit
			else if (tsm.getOptByCTSAndSkill(CharacterTemporaryStat.ReviveOnce, Shade.SUMMON_OTHER_SPIRIT) != null) {
				Shade.reviveBySummonOtherSpirit(chr);
			}

			// Beast Tamer - Bear Reborn		TODO
			else if (tsm.getOptByCTSAndSkill(CharacterTemporaryStat.ReviveOnce, BeastTamer.BEAR_REBORN) != null) {
				BeastTamer.reviveByBearReborn(chr);
			}

			// Zero - Rewind
			else if (tsm.getOptByCTSAndSkill(CharacterTemporaryStat.ReviveOnce, Zero.REWIND) != null) {
				Zero.reviveByRewind(chr);
			}

			// Phantom - Final Feint
			else if (tsm.getOptByCTSAndSkill(CharacterTemporaryStat.ReviveOnce, Phantom.FINAL_FEINT) != null) {
	...
	public void handleHit(Client c, InPacket inPacket, HitInfo hitInfo) {
		...
		if(hitInfo.hpDamage != 0) {
			...
			// Mihile - Soul Link
			else if (tsm.hasStat(CharacterTemporaryStat.MichaelSoulLink) && chr.getId() != tsm.getOption(
                CharacterTemporaryStat.MichaelSoulLink).cOption) {
			...
			// Paladin - Parashock Guard
			else if (tsm.hasStat(CharacterTemporaryStat.KnightsAura) && chr.getId() != tsm.getOption(
                CharacterTemporaryStat.KnightsAura).nOption) {
				Party party = chr.getParty();

				PartyMember paladinInParty = party.getPartyMemberByID(tsm.getOption(CharacterTemporaryStat.KnightsAura).nOption);
				if (paladinInParty != null) {
					...
					int dmgReductionR = si.getValue(SkillStat.y, slv);
