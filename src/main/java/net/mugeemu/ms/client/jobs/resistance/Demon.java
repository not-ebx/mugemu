...
import net.mugeemu.ms.enums.BaseStat;
...
public class Demon extends Job {
    ...
    public void handleBuff(Client c, InPacket inPacket, int skillID, byte slv) {
        ...
        switch (skillID) {
            case OVERLOAD_RELEASE:
                ...
                o2.nOption = (int) (overloadRate * si.getValue(SkillStat.indiePMdR, slv));
                o2.rOption = skillID;
            ...
            case DARK_METAMORPHOSIS:
                ...
                o3.nOption = si.getValue(SkillStat.damage, slv); //?
                o3.rOption = skillID;
                o3.tOption = si.getValue(SkillStat.time, slv);
    ...
    public void diabolicRecoveryHPRecovery() {
		...
		if(tsm.hasStat(CharacterTemporaryStat.DiabolikRecovery)) {
            ...
            int duration = si.getValue(SkillStat.w, slv);
            ...
            if(hasHitMobs) {
                ...
                if(chr.hasSkill(MAX_FURY)) {
                    if(attackInfo.skillId == DEMON_LASH || attackInfo.skillId == DEMON_LASH_2 || attackInfo.skillId == DEMON_LASH_3 || attackInfo.skillId == DEMON_LASH_4) {
                        ...
                        int propz = mfsi.getValue(SkillStat.prop, skillLevel);
        ...
        switch (attackInfo.skillId) {
            ...
            case DEMON_CRY:
                for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                    ...
                    o3.rOption = skillID;
                    o3.tOption = si.getValue(SkillStat.time, slv);
                    o3.xOption = si.getValue(SkillStat.w, slv); // exp
                    o3.yOption = si.getValue(SkillStat.w, slv); // dropRate
            ...
            case THOUSAND_SWORDS:
                ...
                break;
            case CERBERUS_CHOMP:
                int furyabsorbed = si.getValue(SkillStat.x, slv);
                chr.healMP(furyabsorbed);
                break;
            case RAVEN_STORM:
                int hpheal = (int) (chr.getMaxHP() / ((double) 100 / si.getValue(SkillStat.x, slv)));
                chr.heal(hpheal);
                break;
            case VITALITY_VEIL:
                int amounthealed = si.getValue(SkillStat.y, slv);
    ...
    public void lifeSapHealing() {
        ...
        if(chr.hasSkill(LIFE_SAP)) {
            ...
            if(chr.hasSkill(ADVANCED_LIFE_SAP)) {
                amounthealed = SkillData.getSkillInfoById(ADVANCED_LIFE_SAP).getValue(SkillStat.x, chr.getSkill(ADVANCED_LIFE_SAP).getCurrentLevel());
            }
            if(chr.hasSkill(PAIN_DAMPENER)) {
                amounthealed -= SkillData.getSkillInfoById(PAIN_DAMPENER).getValue(SkillStat.x, chr.getSkill(PAIN_DAMPENER).getCurrentLevel());
            }
    ...
    public void leechAuraHealing(AttackInfo attackInfo) {
        ...
        if(chr.hasSkill(LEECH_AURA)) {
            if(tsm.getOptByCTSAndSkill(CharacterTemporaryStat.Regen, LEECH_AURA) != null) {
                ...
                byte slv = (byte) skill.getCurrentLevel();
                SkillInfo si = SkillData.getSkillInfoById(skill.getSkillId());
                int cd = si.getValue(SkillStat.y, slv) * 1000;
                if(cd + leechAuraCD < System.currentTimeMillis()) {
                    for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                        int totaldmg = Arrays.stream(mai.damages).sum();
                        int hpheal = (int) (totaldmg * ((double) 100 / si.getValue(SkillStat.x, slv)));
    ...
    private void applyHpCostForDA(int skillID) {
        ...
        byte slv = (byte) skill.getCurrentLevel();
        SkillInfo si = SkillData.getSkillInfoById(skill.getSkillId());
        int hpRCost = si.getValue(SkillStat.hpRCon, slv);
        if(hpRCost > 0) {
            ...
            switch (skillId) {
                case DEMON_CRY:
                    if (tsm.hasStat(CharacterTemporaryStat.InfinityForce)) {
                        return si.getValue(SkillStat.s, slv) * 1000;
                    }
            }
        }
        return -1;
    ...
    public void handleHit(Client c, InPacket inPacket, HitInfo hitInfo) {
        ...
        if(hitInfo.hpDamage == 0 && hitInfo.mpDamage == 0) {
            ...
            if(chr.hasSkill(POSSESSED_AEGIS)) {
                ...
                byte slv = (byte) skill.getCurrentLevel();
                SkillInfo si = SkillData.getSkillInfoById(skill.getSkillId());
                int propz = si.getValue(SkillStat.x, slv);
                if(Util.succeedProp(propz)) {
                    int mobID = hitInfo.mobID;
