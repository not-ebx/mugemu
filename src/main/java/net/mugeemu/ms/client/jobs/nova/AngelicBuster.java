...
public class AngelicBuster extends Job {
    ...
    public void handleBuff(Client c, InPacket inPacket, int skillID, byte slv) {
        ...
        switch (skillID) {
            case MELODY_CROSS:
                ...
                o2.nOption = si.getValue(SkillStat.mhpX, slv);
                o2.rOption = skillID;
                o2.tOption = si.getValue(SkillStat.time, slv);
                tsm.putCharacterStatValue(CharacterTemporaryStat.EMHP, o2);
                break;
            case POWER_TRANSFER:
                o1.nOption = 1;
                o1.rOption = skillID;
            ...
            case PRETTY_EXALTATION:
                ...
                o2.nReason = skillID;
                o2.nValue = si.getValue(SkillStat.indieBDR, slv);
        ...
        if(hasHitMobs) {
            ...
            if(Util.succeedProp(getRechargeProc(attackInfo))) {
                ...
                if(chr.hasSkill(AFFINITY_HEART_IV) && Util.succeedProp(getRechargeProc(attackInfo))) {
                    ...
                    if(Util.succeedProp(ah4SI.getValue(SkillStat.x, ah4LV))) {
                        rechargeABSkills();
                        affinityHeartIIIcounter = 0;
    ...
    private void soulSeekerExpert(int skillID, byte slv, AttackInfo attackInfo) {
        ...
        if (tsm.hasStat(CharacterTemporaryStat.AngelicBursterSoulSeeker)) {
            ...
            for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                ...
                int TW1prop = si.getValue(SkillStat.prop, slv);
                if(attackInfo.skillId == CELESTIAL_ROAR) {
                    TW1prop += si.getValue(SkillStat.z, slv);
                }
    ...
    private void recreateSoulSeekerForceAtom(AttackInfo attackInfo) {
        ...
        for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
            ...
            int TW1prop = si.getValue(SkillStat.s, slv);
            if (Util.succeedProp(TW1prop)) {
                int mobID = mai.mobId;
    ...
    private int getRechargeProc(AttackInfo attackInfo) {
        ...
        int rechargeproc = rechargeInfo.getValue(SkillStat.onActive, slv);
        if(rechargeproc == 0) {
            return rechargeproc;
        }
        if (chr.hasSkill(AFFINITY_HEART_I)) {
            SkillInfo ah1 = SkillData.getSkillInfoById(AFFINITY_HEART_I);
            int extraRecharge = ah1.getValue(SkillStat.x, slv);
    ...
    private void affinityHeartIV(TemporaryStatManager tsm, byte slv) {
        ...
        if(tsm.getOptByCTSAndSkill(CharacterTemporaryStat.IndieDamR, AFFINITY_HEART_IV) == null) {
            ...
            Option o = new Option();
            o.nValue = si.getValue(SkillStat.y, slv);
