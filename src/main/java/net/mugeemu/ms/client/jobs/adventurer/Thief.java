...
import net.mugeemu.ms.connection.packet.*;
import net.mugeemu.ms.enums.AssistType;
...
public class Thief extends Beginner {
    ...
    public void handleBuff(Client c, InPacket inPacket, int skillID, byte slv) {
        ...
        switch (skillID) {
            ...
            case DARK_SIGHT:
                ...
                break;
            case ASSASSINS_MARK:
                if (tsm.hasStat(CharacterTemporaryStat.NightLordMark)) {
                    ...
                    o1.rOption = skillID;
                    o1.tOption = 0;
                    tsm.putCharacterStatValue(CharacterTemporaryStat.NightLordMark, o1);
                }
                break;
            case CLAW_BOOSTER:
            case DAGGER_BOOSTER:
            ...
            case MIRRORED_TARGET:
                if(tsm.getOptByCTSAndSkill(CharacterTemporaryStat.ShadowPartner, MIRROR_IMAGE) != null) {
            ...
            case BLEED_DART:
                ...
                o1.rOption = skillID;
                o1.tOption = si.getValue(SkillStat.time, slv);
                tsm.putCharacterStatValue(CharacterTemporaryStat.BleedingToxin, o1);
                o2.nReason = skillID;
            ...
            case BLADE_CLONE:
                ...
                o1.rOption = skillID;
                o1.tOption = si.getValue(SkillStat.time, slv);
                tsm.putCharacterStatValue(CharacterTemporaryStat.WindBreakerFinal, o1);
                o2.nOption = 10;
                o2.rOption = skillID;
                o2.tOption = si.getValue(SkillStat.time, slv);
            ...
            case ASURAS_ANGER:
                ...
                o1.rOption = skillID;
                o1.tOption = 10;
                tsm.putCharacterStatValue(CharacterTemporaryStat.Asura, o1);
                break;
        }
    ...
    private void incrementFlipTheCoinStack(TemporaryStatManager tsm, Client c) {
        ...
        if(tsm.hasStat(CharacterTemporaryStat.FlipTheCoin)) {
            ...
            if(amount < FlipTheCoinInfo.getValue(SkillStat.y, 1)) {
                amount++;
            }
        }
        o.nOption = amount;
        o.rOption = FLIP_THE_COIN;
        o.tOption = FlipTheCoinInfo.getValue(SkillStat.time, 1);
        tsm.putCharacterStatValue(CharacterTemporaryStat.FlipTheCoin, o);

        //Stats
        o1.nOption = (amount * FlipTheCoinInfo.getValue(SkillStat.x, 1));
        o1.rOption = FLIP_THE_COIN;
        o1.tOption = FlipTheCoinInfo.getValue(SkillStat.time, 1);
        tsm.putCharacterStatValue(CharacterTemporaryStat.CriticalBuff, o1);
        o2.nReason = FLIP_THE_COIN;
        o2.nValue = (amount * FlipTheCoinInfo.getValue(SkillStat.indieDamR, 1));
        o2.tStart = (int) System.currentTimeMillis();
        o2.tTerm = FlipTheCoinInfo.getValue(SkillStat.time, 1);
        tsm.putCharacterStatValue(CharacterTemporaryStat.IndieDamR, o2);
        o3.nReason = FLIP_THE_COIN;
        o3.nValue = (amount * FlipTheCoinInfo.getValue(SkillStat.indieMaxDamageOver, 1));
        o3.tStart = (int) System.currentTimeMillis();
        o3.tTerm = FlipTheCoinInfo.getValue(SkillStat.time, 1);
        tsm.putCharacterStatValue(CharacterTemporaryStat.IndieMaxDamageOver, o3);
        tsm.sendSetStatPacket();
    }

    private void activateFlipTheCoin(TemporaryStatManager tsm) {    //TODO  Change to proc on Critical Hits
        if(tsm.getOption(CharacterTemporaryStat.FlipTheCoin).nOption < 5) {
    ...
    private void updatecrit() {
        ...
        chr.getTemporaryStatManager().putCharacterStatValue(CharacterTemporaryStat.CriticalGrowing, o);
    ...
    private void incrementShadowInstinct(int skillId, TemporaryStatManager tsm, Client c) {
        ...
        o1.nOption = (amount * InstinctInfo.getValue(SkillStat.kp, slv));
        o1.rOption = SHADOWER_INSTINCT;
        o1.tOption = InstinctInfo.getValue(SkillStat.time, slv);
        tsm.putCharacterStatValue(CharacterTemporaryStat.PAD, o1);
        tsm.sendSetStatPacket();
    }
    ...
    public void giveShadowMeld() {
        ...
        if(chr.hasSkill(SHADOW_MELD)) {
            if(tsm.getOptByCTSAndSkill(CharacterTemporaryStat.IndiePAD, SHADOW_MELD) == null) {
                ...
                if(lastShadowMeld + 5000 < System.currentTimeMillis()) {
                    ...
                    o2.tStart = (int) System.currentTimeMillis();
                    o2.tTerm = si.getValue(SkillStat.time, slv);
                    tsm.putCharacterStatValue(CharacterTemporaryStat.IndiePAD, o2); //Indie
                    tsm.sendSetStatPacket();
    ...
    public void handleAttack(Client c, AttackInfo attackInfo) {
        ...
        if (JobConstants.isShadower(chr.getJob())) {
            if(hasHitMobs) {
                ...
                // Shadower Instinct
                if (chr.hasSkill(SHADOWER_INSTINCT)) {
                    if (tsm.hasStat(CharacterTemporaryStat.IgnoreMobpdpR)) {
        ...
        switch (attackInfo.skillId) {
            ...
            case SHOWDOWN:
                for(MobAttackInfo mai : attackInfo.mobAttackInfo) {
                    ...
                    o2.rOption = skillID;
                    o2.tOption = si.getValue(SkillStat.time, slv);
                    o2.xOption = (mob.isBoss() ? si.getValue(SkillStat.x, slv)/2 : si.getValue(SkillStat.x, slv)); // Exp
                    o2.yOption = (mob.isBoss() ? si.getValue(SkillStat.x, slv)/2 : si.getValue(SkillStat.x, slv)); // Item Drop
            ...
            case FINAL_CUT:
                ...
                o1.rOption = skillID;
                o1.tOption = si.getValue(SkillStat.time, slv);
                tsm.putCharacterStatValue(CharacterTemporaryStat.FinalCut, o1);
                o2.nOption = 1;
                o2.rOption = skillID;
                o2.tOption = 3;
                tsm.putCharacterStatValue(CharacterTemporaryStat.NotDamaged, o2);
    ...
    private void procExpertThrowingStar(int skillId) {
        ...
        if(tsm.getOptByCTSAndSkill(CharacterTemporaryStat.IndieDamR, hideIconSkillId) == null) {
            if(tsm.hasStatBySkillId(hideIconSkillId)) {
                tsm.removeStatsBySkill(hideIconSkillId);
            }
            if(Util.succeedProp(si.getValue(SkillStat.prop, slv))) {
                o.nReason = hideIconSkillId;
                o.nValue = si.getValue(SkillStat.pdR, slv);
            ...
            if(SkillData.getSkillInfoById(skillId) != null) {
                chr.healMP(SkillData.getSkillInfoById(skillId).getValue(SkillStat.mpCon, slv));
            }
    ...
    private int getAssassinsMarkStarCount() {
        if(getMarkSkill() != null) {
            ...
            return si.getValue(SkillStat.bulletCount, slv);
        }
        return 0;
    }
    ...
    private void setMarkonMob(AttackInfo attackInfo) {
        ...
        byte slv = (byte) skill.getCurrentLevel();
        int markprop = si.getValue(SkillStat.prop, slv);
        if(tsm.hasStat(CharacterTemporaryStat.NightLordMark)) {
            for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
    ...
    private void applyBleedDartOnMob(AttackInfo attackInfo) {
        TemporaryStatManager tsm = chr.getTemporaryStatManager();
        if(tsm.hasStat(CharacterTemporaryStat.BleedingToxin)) {
    ...
    private void recoverHPByLifeDrain() {
        if(chr.hasSkill(LIFE_DRAIN)) {
            ...
            SkillInfo si = SkillData.getSkillInfoById(skill.getSkillId());
            int proc = si.getValue(SkillStat.prop, slv);
            int amounthealed = si.getValue(SkillStat.x, slv);
            if(Util.succeedProp(proc)) {
    ...
    private void dropFromPickPocket(AttackInfo attackInfo) {
        ...
        if(tsm.getOptByCTSAndSkill(CharacterTemporaryStat.PickPocket, PICK_POCKET) != null) {
            for(MobAttackInfo mai : attackInfo.mobAttackInfo) {
    ...
    public int getFinalAttackSkill() {
        TemporaryStatManager tsm = chr.getTemporaryStatManager();
        if(tsm.getOptByCTSAndSkill(CharacterTemporaryStat.WindBreakerFinal, BLADE_CLONE) != null) {
            return BLADE_CLONE;
        }
        return 0;
    }
        ...
        if (isBuff(skillID)) {
            ...
            switch (skillID) {
                ...
                case FRAILTY_CURSE:
                    ...
                    int lt1 = si.getValue(SkillStat.lt, slv);
                    int rb1 = si.getValue(SkillStat.rb, slv);
                ...
                case MESO_EXPLOSION:
                    int i = 0;
                    Field field = chr.getField();
                    int rectRange = si.getValue(SkillStat.range, slv);
                    Rect rect = new Rect(
                            new Position(
                    ...
                    for(Drop drop : dropList) {
                        if(drop.isMoney() && (drop.getOwnerID() == chr.getId()) && i < si.getValue(SkillStat.bulletCount, slv)) {
