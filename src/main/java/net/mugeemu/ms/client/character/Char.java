...
import net.mugeemu.ms.client.alliance.AllianceResult;
...
import net.mugeemu.ms.client.character.info.*;
import net.mugeemu.ms.client.character.items.*;
import net.mugeemu.ms.client.character.monsterbattle.MonsterBattleLadder;
import net.mugeemu.ms.client.character.monsterbattle.MonsterBattleMobInfo;
import net.mugeemu.ms.client.character.monsterbattle.MonsterBattleRankInfo;
...
import net.mugeemu.ms.client.friend.FriendFlag;
import net.mugeemu.ms.client.friend.FriendRecord;
import net.mugeemu.ms.client.friend.FriendshipRingRecord;
import net.mugeemu.ms.client.guild.GuildMember;
import net.mugeemu.ms.client.guild.result.GuildResult;
...
import net.mugeemu.ms.client.jobs.resistance.Demon;
import net.mugeemu.ms.client.jobs.resistance.WildHunterInfo;
import net.mugeemu.ms.client.jobs.sengoku.Kanna;
import net.mugeemu.ms.client.party.Party;
import net.mugeemu.ms.client.party.PartyMember;
import net.mugeemu.ms.client.party.PartyResult;
import net.mugeemu.ms.connection.OutPacket;
import net.mugeemu.ms.connection.db.DatabaseManager;
import net.mugeemu.ms.connection.db.InlinedIntArrayConverter;
...
import net.mugeemu.ms.handlers.ClientSocket;
import net.mugeemu.ms.handlers.EventManager;
import net.mugeemu.ms.handlers.PsychicLock;
import net.mugeemu.ms.life.*;
...
import net.mugeemu.ms.scripts.ScriptInfo;
...
import net.mugeemu.ms.world.field.fieldeffect.FieldEffect;
import net.mugeemu.ms.world.gach.GachaponManager;
...
public class Char {
	...
	public Char() {
		this(new Account(), "", 0, 0, 0, (short) 0, (byte) -1, (byte) -1, 0, 0, new int[]{});
	}
	...
	public static Char getFromDBByName(String name) {
		...
		return udao.getCharacterByName(name);
	}

	public static Char findCharacterByName(String name) {
		return getFromDBByName(name);
	}
		...
		if (inventory != null) {
			...
			if (existingItem != null && existingItem.getInvType().isStackable() && existingItem.getQuantity() < ii.getSlotMax()) {
				...
				write(WvsContext.inventoryOperation(true, false,
						InventoryOperation.UpdateQuantity, (short) existingItem.getBagIndex(), (byte) -1, 0, existingItem));
				...
				write(WvsContext.inventoryOperation(true, false,
                        InventoryOperation.Add, (short) item.getBagIndex(), (byte) -1, 0, item));
				if (rec) {
	...
	public void encode(OutPacket outPacket, DBChar mask) {
		...
		if(somethingWeirdValue) {
			byte unknownVal = 0;
			for (byte i = 0; i < unknownVal; i++){
				outPacket.encodeInt(0); // ??
			}
			int unknownInt = 0;
			outPacket.encodeInt(unknownInt);
			for (int i = 0; i < unknownInt; i++){
				outPacket.encodeInt(0);
				outPacket.encodeLong(0);
			}
			byte anotherByte = 0;
			outPacket.encodeByte(anotherByte);
			if(anotherByte > 0){
				outPacket.encodeByte(0);
				outPacket.encodeInt(0);
				for (byte i = 0; i < anotherByte; i++) {
					outPacket.encodeLong(0);
				}
				int anotherInt = 0;
				outPacket.encodeInt(anotherInt);
				for (int i = 0; i < anotherInt; i++) {
					outPacket.encodeLong(0);
				}
			}
		...
		// Check this part, it generates a big int which makes it enter a loop.
		...
		if(mask.isInMask(DBChar.EquipExt)) {
			outPacket.encodeFT(-2); //expire time
		}
		...
		if (mask.isInMask(DBChar.ItemSlotEquip)) {
			...
			for (Item item : equippedItems) {
				...
				if (item.getBagIndex() > BodyPart.BPBase.getVal() && item.getBagIndex() < BodyPart.BPEnd.getVal()) {
					outPacket.encodeShort((short)equip.getBagIndex());
		...
		if (mask.isInMask(DBChar.SkillCooltime)) {
			...
			for (Map.Entry<Integer, Long> cooltime : cooltimes.entrySet()) {
				...
				outPacket.encodeShort((int) ((cooltime.getValue() - curTime) / 1000)); // nSkillCooltime was int
			}
		}
	...
	public void chatMessage(String msg, Object... args) {
		chatMessage(ChatType.SystemNotice, msg, args);
	}
	...
	public boolean equip(Item item) {
		...
		AvatarLook al = getAvatarData().getAvatarLook();
		int itemID = item.getItemId();
		getInventoryByType(InvType.EQUIP).removeItem(item);
		getInventoryByType(InvType.EQUIPPED).addItem(item);
	...
	public void warp(Field toField, Portal portal, boolean characterData, boolean saveReturnMap) {
		...
		getClient().write(
			Stage.setField(this, toField, getClient().getChannel(),
				false,
				0,
				characterData,
				hasBuffProtector(),
				(byte) (portal != null ? portal.getId() : 0),
				false,
				100,
				null,
				true,
		...
		*/
		...
		/*if (tsm.hasStat(CharacterTemporaryStat.IndieEmpty)) {
			for (Iterator<Option> iterator = tsm.getCurrentStats().getOrDefault(CharacterTemporaryStat.IndieEmpty, new ArrayList<>()).iterator(); iterator.hasNext(); ) {
	...
	public void consumeItem(Item item) {
		...
		if (item.getQuantity() <= 1 && !ItemConstants.isThrowingItem(item.getItemId())) {
			...
			write(WvsContext.inventoryOperation(true, false,
					InventoryOperation.Remove, bagIndex, (byte) 0, 0, item));
		} else {
			item.setQuantity(item.getQuantity() - 1);
			write(WvsContext.inventoryOperation(true, false,
					InventoryOperation.UpdateQuantity, (short) item.getBagIndex(), (byte) -1, 0, item));
		}
		setBulletIDForAttack(calculateBulletIDForAttack(1));
	}
	...
	public void logout() {
		//TODO Change the wholelogic wtf
		punishLieDetectorEvasion();
		log.info("Logging out " + getName());
		if (getField() != null && getField().getForcedReturn() != GameConstants.NO_MAP_ID) {
		...
		} else if (getClient() != null && getClient().getUser() != null) {
		...
		DatabaseManager.merge(getAccount());
	}
	...
	public void addItemToInventory(int id, int quantity) {
		if (ItemConstants.isEquip(id)) {  //Equip
			...
			getClient().write(WvsContext.inventoryOperation(true, false,
                    InventoryOperation.Add, (short) equip.getBagIndex(), (byte) -1, 0, equip));
	...
	public void failedLieDetector() {
		...
		chatMessage(ChatType.SpeakerChannel, "You have failed the Lie Detector test.");
	...
	public void passedLieDetector() {
		setLieDetectorAnswer("");
		chatMessage(ChatType.SpeakerChannel, "You have passed the Lie Detector test!");
	...
	public boolean applyBulletCon(int skillID, byte slv) {
		if (getTemporaryStatManager().hasStat(CharacterTemporaryStat.NoBulletConsume) || JobConstants.isPhantom(getJob())) {
			return true;
		}
		SkillInfo si = SkillData.getSkillInfoById(skillID);
		if (si == null) {
	...
	public void initBlessingSkillNames() {
		UserDAO udao = new UserDAO();
		List<Char> accCharacters = udao.getSelectCharacterFromAccount(account);
		Char fairyChar = null;
		for (Char chr : accCharacters) {
			if (!chr.equals(this)
					&& chr.getLevel() >= 10
	...
	public void initBlessingSkills() {
		UserDAO udao = new UserDAO();
		// TODO: Is this secure?
		Char fairyChar = udao.getCharacterByName(getBlessingOfFairy());
		if (fairyChar != null) {
			addSkill(SkillConstants.getFairyBlessingByJob(getJob()),
					Math.min(20, fairyChar.getLevel() / 10), 20);
		}
		// TODO: Is this secure?
		Char empressChar = udao.getCharacterByName(getBlessingOfEmpress());
		if (empressChar != null) {
	...
	public void increaseGolluxStack() {
		int maxStack = 5;
		TemporaryStatManager tsm = getTemporaryStatManager();
		int stack = tsm.getCurrentStats().get(CharacterTemporaryStat.Stigma) != null ? tsm.getCurrentStats().get(
			CharacterTemporaryStat.Stigma).get(0).nOption : 0;
		stack++;
		Option o = new Option();
		if (stack >= maxStack) {
			this.damage(getHP());
		...
		o.rOption = 800;
		o.bOption = maxStack;
		tsm.putCharacterStatValue(CharacterTemporaryStat.Stigma, o);
