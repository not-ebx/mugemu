...
import net.mugeemu.ms.connection.crypto.ShandaCrypto;
    ...
    protected void decode(ChannelHandlerContext chc, ByteBuf in, List<Object> out) {
        ...
        if (c != null) {
            ...
            if (in.readableBytes() >= c.getStoredLength()) {
                ...
                byte[] decChannel = mCr.crypt(dec, iv);
                c.setRecvIV(MapleCrypto.getNewIv(iv));

                byte[] decShanda = ShandaCrypto.decrypt(decChannel);
                InPacket inPacket = new InPacket(decShanda);
                out.add(inPacket);
            }
        }
    }
