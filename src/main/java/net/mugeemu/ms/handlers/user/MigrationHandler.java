...
import net.mugeemu.ms.enums.MapTransferType;
...
import net.mugeemu.ms.loaders.NpcData;
import net.mugeemu.ms.scripts.ScriptType;
import net.mugeemu.ms.util.container.Tuple;
import net.mugeemu.ms.world.event.InGameEventManager;
import net.mugeemu.ms.world.field.Field;
import net.mugeemu.ms.world.field.FieldInstanceType;
...
public class MigrationHandler {
    ...
    public static void handleMigrateIn(Client c, InPacket inPacket) {
        ...
        int charId = inPacket.decodeInt();// Decodes 5
        byte[] machineID = inPacket.decodeArr(16); // Literal machine id xdd

        Char migratingCharacter = udao.getCharacterById(charId);
        Account migratingAccount = udao.getAccountById(migratingCharacter.getAccId());

        int worldId = migratingAccount.getWorldId();
...
//            chr.warp(chr.getOrCreateFieldByCurrentInstanceType(chr.getFieldID()));
        ...
        if (portalName != null && !portalName.isEmpty()) {
            ...
            byte reviveType = inPacket.decodeByte(); // 01?
