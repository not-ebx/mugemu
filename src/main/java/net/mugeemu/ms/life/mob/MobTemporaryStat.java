...
import net.mugeemu.ms.connection.packet.BattleRecordMan;
import net.mugeemu.ms.connection.packet.MobPool;
...
public class MobTemporaryStat {
	...
	public void createAndAddBurnedInfo(Char chr, Skill skill) {
		...
		int time = si.getValue(SkillStat.dotTime, slv) * 1000;
		if (bi == null) {
			...
			bi.setInterval(si.getValue(SkillStat.dotInterval, slv) * 1000);
			bi.setDotCount(time / bi.getInterval());
			bi.setSuperPos(si.getValue(SkillStat.dotSuperpos, slv));
			bi.setAttackDelay(0);
			bi.setDotTickIdx(0);
