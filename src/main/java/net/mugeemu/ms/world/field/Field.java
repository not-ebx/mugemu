...
import net.mugeemu.ms.scripts.ScriptManager;
...
public class Field {
    ...
    public void spawnAffectedArea(AffectedArea aa) {
        ...
        if (si != null || (aa.getMobOrigin() > 0 && msi != null)) {
            ...
            if (si != null) {
                duration = aa.getDuration() == 0 ? si.getValue(SkillStat.time, aa.getSlv()) * 1000 : aa.getDuration();
            }
            if (duration > 0) {
    ...
    public synchronized void removeDrop(int dropID, int pickupUserID, boolean fromSchedule, int petID) {
        ...
        if (life instanceof Drop) {
            if (petID >= 0) {
                broadcastPacket(DropPool.dropLeaveField(
                    DropLeaveType.PetPickup, pickupUserID, life.getObjectId(),
                        (short) 0, petID, 0));
